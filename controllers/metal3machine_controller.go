...
func (r *Metal3MachineReconciler) reconcileNormal(ctx context.Context,
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("reconcileNormal: starting")
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Checking machine provisioning state",
		"isProvisioned", isProvisioned)

	if isProvisioned && machineMgr.MachineHasNodeRef() {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Machine is provisioned with NodeRef")
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Updating provisioned machine")
		...
		if err != nil {
			r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to update provisioned machine",
				baremetal.LogFieldError, err.Error())
			errType := capierrors.UpdateMachineError
			return checkMachineError(machineMgr, err,
				"Failed to update the Metal3Machine", errType)
		}
		r.Log.V(baremetal.VerbosityLevelTrace).Info("reconcileNormal: provisioned machine updated successfully")
		return ctrl.Result{}, nil
	}
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Checking bootstrap readiness",
		"bootstrapReady", bootstrapReady)

	if !bootstrapReady {
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Bootstrap not ready, waiting")
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Checking BMH association",
		"hasBMHAnnotation", hasAnnotation)

	if !hasAnnotation {
		r.Log.V(baremetal.VerbosityLevelTrace).Info("No BMH annotation, attempting to associate")
		// Associate the baremetalhost hosting the machine
		err := machineMgr.Associate(ctx)
		if err != nil {
			r.Log.V(baremetal.VerbosityLevelDebug).Info("Association failed",
				baremetal.LogFieldError, err.Error())
		...
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Association initiated successfully")
		return ctrl.Result{}, nil
	}

	r.Log.V(baremetal.VerbosityLevelTrace).Info("BMH already associated, updating conditions")
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Associating Metal3 metadata")
	err := machineMgr.AssociateM3Metadata(ctx)
	if err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Metadata association failed",
			baremetal.LogFieldError, err.Error())
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Updating machine after metadata association")
	err = machineMgr.Update(ctx)
	if err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to update BareMetalHost",
			baremetal.LogFieldError, err.Error())
		return checkMachineError(machineMgr, err,
			"failed to update BareMetalHost", errType)
	}

	r.Log.V(baremetal.VerbosityLevelTrace).Info("Checking if BMH is provisioned")
	if !machineMgr.IsBaremetalHostProvisioned(ctx) {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("BareMetalHost not yet provisioned, waiting")
		r.Log.Info("IsBaremetalHostProvisioned check failed")
		return ctrl.Result{}, nil
	}
	r.Log.V(baremetal.VerbosityLevelDebug).Info("BareMetalHost is provisioned")

	r.Log.V(baremetal.VerbosityLevelTrace).Info("Checking if Node with matching ProviderID exists")
	if machineMgr.NodeWithMatchingProviderIDExists(ctx, r.CapiClientGetter) {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Node with matching ProviderID found, waiting for NodeRef")
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Checking cloud provider status",
		"cloudProviderEnabled", cloudProviderEnabled)

	if cloudProviderEnabled {
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Cloud provider enabled, setting ProviderID from cloud provider node")
		err = machineMgr.SetProviderIDFromCloudProviderNode(ctx, r.CapiClientGetter)
		if err != nil {
			r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to set ProviderID from cloud provider",
				baremetal.LogFieldError, err.Error())
	...
	// caught it above.
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Attempting to set ProviderID from node label")
	success, err := machineMgr.SetProviderIDFromNodeLabel(ctx, r.CapiClientGetter)
	if err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to set ProviderID from node label",
			baremetal.LogFieldError, err.Error())
		return checkMachineError(machineMgr, err, "failed to set ProviderID on Metal3Machine based on Node label", errType)
	}

	r.Log.V(baremetal.VerbosityLevelDebug).Info("SetProviderIDFromNodeLabel result",
		baremetal.LogFieldResult, success)

	if success {
		r.Log.V(baremetal.VerbosityLevelTrace).Info("ProviderID set from node label, marking ready")
		machineMgr.SetReadyTrue()
		return ctrl.Result{}, nil
	}

	hasProviderID := machineMgr.Metal3MachineHasProviderID()
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Checking if Metal3Machine has ProviderID",
		"hasProviderID", hasProviderID)

	if !hasProviderID {
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Setting default ProviderID")
		err = machineMgr.SetDefaultProviderID()
		if err != nil {
			r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to set default ProviderID",
				baremetal.LogFieldError, err.Error())
			return checkMachineError(machineMgr, err,
		...
		errType = capierrors.UpdateMachineError
		r.Log.V(baremetal.VerbosityLevelTrace).Info("Updating machine after setting default ProviderID")
		err = machineMgr.Update(ctx)
		if err != nil {
			r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to update Metal3Machine",
				baremetal.LogFieldError, err.Error())
			return checkMachineError(machineMgr, err,
				"Failed to update the Metal3Machine", errType)
		}
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Setting node ProviderID by hostname")
	err = machineMgr.SetNodeProviderIDByHostname(ctx, r.CapiClientGetter)
	if err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to set node ProviderID by hostname",
			baremetal.LogFieldError, err.Error())
		errType = capierrors.UpdateMachineError
	...
	errType = capierrors.UpdateMachineError
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Final machine update")
	err = machineMgr.Update(ctx)
	if err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Final update failed",
			baremetal.LogFieldError, err.Error())
		return checkMachineError(machineMgr, err,
			"Failed to update the Metal3Machine", errType)
	}

	r.Log.V(baremetal.VerbosityLevelTrace).Info("reconcileNormal: completed successfully")
	return ctrl.Result{}, nil
}
...
func (r *Metal3MachineReconciler) reconcileDelete(ctx context.Context,
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("reconcileDelete: starting deletion workflow")

	// set machine condition to Deleting
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Setting deletion conditions on Metal3Machine")
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Deletion conditions set",
	...
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Dissociating M3Metadata from machine")
	if err := machineMgr.DissociateM3Metadata(ctx); err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to dissociate M3Metadata",
			baremetal.LogFieldError, err.Error())
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("M3Metadata dissociated successfully")

	// delete the machine
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Deleting Metal3Machine resources (BMH dissociation)")
	if err := machineMgr.Delete(ctx); err != nil {
		r.Log.V(baremetal.VerbosityLevelDebug).Info("Failed to delete Metal3Machine resources",
			baremetal.LogFieldError, err.Error())
	...
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Metal3Machine resources deleted successfully")

	// metal3machine is marked for deletion and ready to be deleted,
	// so remove the finalizer.
	r.Log.V(baremetal.VerbosityLevelTrace).Info("Removing finalizer from Metal3Machine")
	machineMgr.UnsetFinalizer()
	r.Log.V(baremetal.VerbosityLevelDebug).Info("Finalizer removed")

	r.Log.V(baremetal.VerbosityLevelTrace).Info("reconcileDelete: completed successfully")
	return ctrl.Result{}, nil
}
